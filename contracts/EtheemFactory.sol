...
contract EtheemFactory is AragonApp {
  ...
  enum Apps { Finance, TokenManager, Vault, Voting, Etheem }
    ...
    returns (Etheem)
  {
    ...
    Etheem etheem = Etheem(dao.newAppInstance(appIds[uint8(Apps.Etheem)], latestVersionAppBase(appIds[uint8(Apps.Etheem)])));
    InstalledApp(etheem, appIds[uint8(Apps.Etheem)]);

    token.changeController(tokenManager); // sender has to create tokens

    // permissions
    acl.createPermission(ANY_ENTITY, voting, voting.CREATE_VOTES_ROLE(), etheem);
    acl.createPermission(voting, voting, voting.MODIFY_QUORUM_ROLE(), etheem);

    acl.createPermission(finance, vault, vault.TRANSFER_ROLE(), etheem);
    acl.createPermission(etheem, finance, finance.CREATE_PAYMENTS_ROLE(), etheem);
    acl.createPermission(etheem, finance, finance.EXECUTE_PAYMENTS_ROLE(), etheem);
    acl.createPermission(etheem, finance, finance.DISABLE_PAYMENTS_ROLE(), etheem);
    acl.createPermission(etheem, tokenManager, tokenManager.ASSIGN_ROLE(), etheem);
    acl.createPermission(etheem, tokenManager, tokenManager.REVOKE_VESTINGS_ROLE(), etheem);
    acl.createPermission(msg.sender, etheem, etheem.CREATE_AUTHOR_ROLE(), etheem);
    ...
    // etheem is returned so init can happen later
    return etheem;
  }
